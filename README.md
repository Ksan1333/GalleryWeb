...
現在版の全体仕様は[アプリ仕様書 v0.2.4](docs/APP_SPECIFICATION_v0.2.4.md)を参照してください。
...
このワークスペースで生成済みの配布物は
`release/0.2.4/PixVault for Windows_0.2.4_x64-setup.exe` です。SHA-256は
`5D63411E136744E9C76EE517DDC8A3BA569857DA8E266ECB81B798C2D3862D08` です。
...
初回起動後は「全フォルダー」から場所を開くか、「優先フォルダーを追加」で先に読み込む場所を選びます。画像、GIF、動画、PDF、ZIP/CBZがライブラリに表示され、元ファイルは移動・複製されません。

0.2.4では、インストール後にエクスプローラーの「プログラムから開く」でPixVaultを選べます。Windowsの既定のアプリでPixVaultを選択した形式は、次回からダブルクリックで直接開けます。画像・GIF・WebP・動画・ZIP/CBZ・PDFに対応し、起動済みの場合も同じウィンドウで開きます。指定ファイルだけを読み込むため、そのフォルダー全体のスキャン完了を待つ必要はありません。HEIC/HEIFと動画の一部コンテナー・コーデックは、Windows/WebView2側のデコーダーがない環境では内容を再生できない場合があります。
